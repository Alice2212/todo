...
const updateTodo = (event) => {
  ...
  const todoId = event.target.parentElement.id.replace("todos-", "");
  console.log(todoId);

  // Take input from user
  const updatedTodo = prompt("Edit your task");
  ...
  if (todoIndex !== -1) {
    ...
    // Store updated todos in local storage
    storeTodo(todoDatabase);

    // Clear existing todo items from the display
    const todoList = document.querySelector("#todos");
    todoList.innerHTML = "";

    // Display updated todos
    todoDatabase.forEach((todo) => {
      displayNewTodo("#todos", todo.todoName, todo.id);
    });
  }
};
